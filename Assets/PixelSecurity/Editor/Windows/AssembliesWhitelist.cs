...
namespace PixelSecurity.Editor.Windows
{
    /// <summary>
    /// Assemblies Whitelist Editor
    /// </summary>
    public class AssembliesWhitelist : EditorWindow
    {
        ...
        private void OnGUI()
		{
			...
			GUILayout.Label("User-defined Whitelist of Assemblies trusted by Injection Protector", tmpStyle);
			...
			if (GUILayout.Button("Add Assemblies from folder"))
			{
				...
				{
					string[] libraries = WizardUtils.FindLibrariesAt(selectedFolder);
		...
		{
			whitelistPath = WizardUtils.ResolveInjectionUserWhitelistPath();
			if (!String.IsNullOrEmpty(whitelistPath) && File.Exists(whitelistPath))
			{
				string[] separator = { GlobalConstants.INJECTION_DATA_SEPARATOR };
				...
				for (int i = 0; i < count; i++)
				{
					...
					{
						Debug.LogWarning("Error parsing whitelist file line! Please report to " + GlobalConstants.REPORT_EMAIL);
					}
				}
		...
		private void WriteWhiteList()
		{
			if (whiteList.Count > 0)
			{
				...
				WizardUtils.RemoveReadOnlyAttribute(whitelistPath);
				...
				foreach (AllowedAssembly assembly in whiteList)
				{
					...
					string line = SecuredString.EncryptDecrypt(assemblyName + GlobalConstants.INJECTION_DATA_SEPARATOR + hashes, "TinyPlay");
					br.Write(line);
				}
				br.Close();
				fs.Close();
